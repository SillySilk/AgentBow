...
impl BrowserBridge {
    ...
    /// HTML mode is automatically distilled — scripts/styles stripped, content truncated.
    pub async fn read_page(&self, mode: &str) -> Result<Value> {
        ...
        let content = match mode {
            "html" => distill_html(&raw_content),
            "text" => truncate_text(&raw_content, 8000),
            _ => raw_content,
        };
    ...
    /// Return all bookmarks as a flat list with folder path, title, and URL.
    pub async fn get_bookmarks(&self) -> Result<Value> {
        let cmd = json!({ "type": "browser_cmd", "cmd": "get_bookmarks" });
        let result = self.send_and_wait(cmd).await?;
        Ok(json!({
    ...
    /// Capture screenshot + distilled page text in one call.
    pub async fn analyze_page(&self) -> Result<Value> {
        // Run both in parallel
        let (shot_result, page_result) = tokio::join!(
            self.screenshot(),
            self.read_page("text")
        );

        let screenshot_b64 = shot_result.ok().and_then(|v| {
            v.as_array()?.first()?.get("source")?.get("data")?.as_str().map(|s| s.to_string())
        });

        let (url, title, text_content) = match page_result {
            Ok(v) => (
        ...
        if let Some(b64) = screenshot_b64 {
            out["screenshot_base64"] = json!(b64);
            out["screenshot_note"] = json!("PNG screenshot available if your model supports vision.");
        }
...
fn distill_html(html: &str) -> String {
...
fn simple_strip_html(html: &str) -> String {
...
fn truncate_text(s: &str, max_chars: usize) -> String {
    ...
    let prefix = crate::util::char_prefix(s, max_chars);
